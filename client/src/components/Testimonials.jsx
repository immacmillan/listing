...
const SWIPE_THRESHOLD = 45 // px of horizontal travel needed to change review
...
export default function Testimonials() {
 ...
 const onTouchMove = (e) => {
   ...
   // scroll, so we never hijack the page scroll.
   ...
   // Rubber-band the card with the finger, damped past the threshold
   setDragX(Math.abs(dx) > SWIPE_THRESHOLD ? dx * 0.35 : dx * 0.6)
 }
 ...
 return (
   <section id="testimonials" className="scroll-mt-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 border-t border-gray-200">
     <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
       ...
       {/* Nav arrows — desktop only; mobile & tablet swipe instead */}
       <div className="hidden lg:flex gap-2 self-start sm:self-auto">
         <button
     ...
     <div
       className="relative bg-white border border-gray-200 rounded-2xl p-6 sm:p-8 shadow-sm min-h-[220px] touch-pan-y select-none lg:select-auto"
       style={{
       ...
       aria-label={`Guest review ${active + 1} of ${reviews.length}`}
     >
       {/* Quote mark */}
     ...
     {/* Review count summary — swipe hint on touch sizes */}
