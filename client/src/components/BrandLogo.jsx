...
* + "A&M Luxe Listings" in italic serif lettering with a staggered,
* semi-futuristic blink-in.
*
* Lettering: Cormorant Garamond Medium Italic (Google Fonts). Letters blink
* in individually and stack up left to right; words are kept in their own
* spans so spacing and sizing stay controlled.
*
* @param {boolean} dark  true when the navbar is in its solid/white state
*/
export default function BrandLogo({ dark }) {
 // All segments share one font size so cap heights — and therefore the tops
 // of the letters — line up across "A&M" and "Luxe Listings". A&M is set
 // apart by weight, a hairline stroke, and colour rather than by size.
 const segments = [
   {
     text: 'A&M',
     className: `text-[1.6rem] font-bold mr-[0.3rem] ${dark ? 'text-orange-700' : 'text-orange-400'}`,
     // Cormorant Garamond stops at 700, so thicken optically to read as
     // genuinely bold against the lighter wordmark.
     style: { WebkitTextStroke: '0.6px currentColor' },
   },
   { text: 'Luxe', className: 'text-[1.6rem] font-semibold' },
   { text: 'Listings', className: 'text-[1.6rem] font-semibold' },
 ]
 ...
 return (
   <span className="inline-flex items-center gap-2.5 select-none">
     ...
     <span
       ...
       role="text"
       className="inline-flex items-baseline gap-[0.4rem] leading-none italic"
       ...
       {(() => {
         ...
         return segments.map((seg) => (
           <span
             key={seg.text}
             aria-hidden="true"
             className={`whitespace-nowrap ${seg.className}`}
             style={seg.style}
           >
